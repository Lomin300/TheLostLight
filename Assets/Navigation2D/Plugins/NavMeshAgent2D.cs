...
public class NavMeshAgent2D : Bolt.EntityBehaviour<IICustomCanvas>
{
    ...
    public bool CalculatePath(Vector2 targetPosition, NavMeshPath2D path)
    {
        ...
        if (agent.CalculatePath(NavMeshUtils2D.ProjectTo3D(targetPosition), temp) &&entity.IsOwner)
        {
            // convert 3D to 2D
    ...
    public void Warp(Vector2 v)
    {
        ...
        if (agent.Warp(NavMeshUtils2D.ProjectTo3D(v)) && entity.IsOwner)
